...
class BasicRegression(BaseEstimator):
    ...
    def _add_penalty(self, loss, w):
        """Apply regularization to loss. (lambda)"""
        if self.penalty == "l1":
    ...
    def _gradient_descent(self):
        ...
        for i in range(1, self.max_iters + 1):
            ...
            delta = cost_d(theta)  # Check Line
            theta -= self.lr * delta

            errors.append(self._cost(self.X, self.y, theta))
            logging.info('Interation %s, error %s' % (i, errors[i]))

            error_diff = np.linalg.norm(errors[i - 1] - errors[i])
            if errors_diff < self.tolerance:
                logging.info('Convergence has been reached.')
                break
        return theta, errors
...
class LinearRegression(BasicRegression):
    ...
    def init_cost(self):
        self.cost_func = mean_squared_error  # where is mean squared error?


class LogisticRegression(BasicRegression):
    """Binary logistic regression with gradient descent optimzer."""

    def init_cost(self):
        self.cost_func = binary_crossentropy  # Where is binary_crossentropy?
